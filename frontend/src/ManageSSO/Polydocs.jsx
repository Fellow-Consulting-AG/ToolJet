...
export function Polydocs({ settings, updateData }) {
  ...
  const changeStatus = () => {
    setSaving(true);
    organizationService.editOrganizationConfigs({ type: 'polydocs', enabled: !enabled }).then(
      (data) => {
        setSaving(false);
        const enabled_tmp = !enabled;
        setEnabled(enabled_tmp);
        data.id && setConfigId(data.id);
        updateData('polydocs', { id: data.id, enabled: enabled_tmp });
        toast.success(`${enabled_tmp ? 'Enabled' : 'Disabled'} Polydocs SSO`, {
          position: 'top-center',
        });
      },
      () => {
